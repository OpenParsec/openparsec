...
{
	/*fixed_t translation;
	if ( AcFetchIntParams( 1, &translation ) ) {
...
#else
	GenObject *obj = CreateObject( objclass, startmatrx, LocalPlayerId );
#endif // PARSEC_SERVER
	if ( obj == NULL ) {
...
void AcCreateObject()
{
	...
	if ( AcFetchIntParams( 1, &objclass ) ) {
		...
		if ( AUX_NO_DELAYED_CREATE_OBJECT_REPLAY ) {
			...
			delayed_object_id		= -1; // not yet set
			delayed_object_hostid	= -1; // not yet set
		}
	}*/
}
...
/*	int32 keynum;
	if ( AcFetchIntParams( 1, &keynum ) ) {
		if ( keynum < sizeof( keyfunc_s ) ) {

			ASSERT( sizeof( DepressedKeys->key_Escape ) == sizeof( byte ) );
			((byte*)DepressedKeys)[ keynum ]  = 1;
			((byte*)key_replay_map)[ keynum ] = 1;
...
/*
	...
	if ( AcFetchIntParams( 1, &keynum ) ) {
		if ( keynum < sizeof( keyfunc_s ) ) {
			ASSERT( sizeof( DepressedKeys->key_Escape ) == sizeof( byte ) );
			((byte*)DepressedKeys)[ keynum ]  = 0;
			((byte*)key_replay_map)[ keynum ] = 0;
		} else {
			CON_AddLine( invalid_key_id );
		}
	}
...
/*
	...
	if ( AcFetchIntParams( 4, params ) ) {
		...
		LightColorAmbient.R = params[ 0 ];
		LightColorAmbient.G = params[ 1 ];
		LightColorAmbient.B = params[ 2 ];
		LightColorAmbient.A = params[ 3 ];
	}
*/
}
...
/*
	...
	if ( AcFetchIntParams( 4, params ) ) {
		...
		LightColorDiffuse.R = params[ 0 ];
		LightColorDiffuse.G = params[ 1 ];
		LightColorDiffuse.B = params[ 2 ];
		LightColorDiffuse.A = params[ 3 ];
	}
*/
}
...
/*
	...
	if ( AcFetchIntParams( 4, params ) ) {
		...
		LightColorSpecular.R = params[ 0 ];
		LightColorSpecular.G = params[ 1 ];
		LightColorSpecular.B = params[ 2 ];
		LightColorSpecular.A = params[ 3 ];
	}
*/
}
...
/*
	...
	for ( int cid = 0; cid < num_scheduled_commands; cid++ ) {
